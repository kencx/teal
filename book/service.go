...
type Store interface {
	...
	RetrieveBooksFromAuthor(name string) ([]*teal.Book, error)
}
...
func (s *Service) Update(ctx context.Context, id int, b *teal.Book) (*teal.Book, error) {
	book, err := s.db.UpdateBook(ctx, id, b)
	if err != nil {
		return nil, err
	}
...
func (s *Service) GetByAuthor(name string) ([]*teal.Book, error) {
	return s.db.RetrieveBooksFromAuthor(name)
}
